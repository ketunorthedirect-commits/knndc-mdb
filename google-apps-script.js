...
// ════════════════════════════════════════════════════════════
//  MIGRATION — Fix Users sheet headers if created by old version
//  Run this manually if Users tab is missing columns or not saving
// ════════════════════════════════════════════════════════════
function migrateUsersSheet() {
  ...
  if (!sheet) {
    _setupUsersSheet(ss);
    SpreadsheetApp.getUi().alert('✅ Users sheet did not exist — created with all 12 columns.');
    return;
  }

  // Check current header row (row 2)
  const lastCol  = Math.max(sheet.getLastColumn(), 1);
  const existHdrs = sheet.getRange(2, 1, 1, lastCol).getValues()[0].map(h => String(h).trim());
  const required  = [
    'User ID','Full Name','Username','Password','Role',
    'Ward','Station Code','Branch','Assigned Stations',
    'Status','Must Change Password','Last Updated'
  ...
  let added = 0;
  required.forEach((hdr, i) => {
    const col = i + 1; // 1-based
    if (existHdrs[i] !== hdr) {
      // Insert the correct header value (doesn't touch data below)
      sheet.getRange(2, col).setValue(hdr)
        .setFontWeight('bold').setBackground('#7f1d1d').setFontColor(WHITE);
      added++;
    }
  });

  // Ensure sheet has at least 12 columns wide
  if (sheet.getMaxColumns() < 12) {
    sheet.insertColumnsAfter(sheet.getMaxColumns(), 12 - sheet.getMaxColumns());
  }

  // Re-apply header formatting to all 12
  sheet.getRange(2, 1, 1, 12).setFontWeight('bold').setBackground('#7f1d1d').setFontColor(WHITE);

  // Fix title row merge to cover all 12 columns
  try { sheet.getRange('A1:L1').merge(); } catch(_) {}
  sheet.getRange('A1').setValue('SYSTEM USERS')
    .setFontSize(14).setFontWeight('bold').setHorizontalAlignment('center')
    .setBackground(NDC_RED).setFontColor(WHITE);

  sheet.setFrozenRows(2);
  [90,180,130,130,90,130,110,140,200,80,140,160].forEach((w,i)=>sheet.setColumnWidth(i+1,w));

  const msg = added > 0
    ? `✅ Users sheet headers fixed (${added} column(s) updated). Columns are now correct.`
    : '✅ Users sheet headers are already correct — no changes needed.';

  Logger.log(msg);
  SpreadsheetApp.getUi().alert(msg + '\n\nNow click "👥 Sync Users from Sheet" in the app, or save any user to push data.');
}
...
function _saveUsers(data) {
  ...
  // ── Self-healing: ensure sheet has correct 12-column headers ──
  const requiredHdrs = [
    'User ID','Full Name','Username','Password','Role',
    'Ward','Station Code','Branch','Assigned Stations',
    'Status','Must Change Password','Last Updated'
  ...
  // Write/fix header row 2
  sheet.getRange(2, 1, 1, 12).setValues([requiredHdrs])
    .setFontWeight('bold').setBackground('#7f1d1d').setFontColor(WHITE);
